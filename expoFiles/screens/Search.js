...
export default class Search extends Component {
  ...
  componentDidMount = async() =>{
    ...
    const url = 'http://***.ca/app/searchLists.php';
  ...
  initiateSearch = async() => {
    ...
    this.props.navigation.navigate('ListResults', { searchTerm: searchTerm, level: this.state.level, format: this.state.format, searchType: this.state.searchType } );
  }
  ...
  renderItem = (item) => {
    ...
    return (
      <View behavior='padding' style={ Stylesheet.outerContainer }>
        ...
        <TouchableOpacity style={ Stylesheet.btnFormat } onPress={ () => navigate('ListResults', { searchTerm: item.RptNumber, level: this.state.level, format: this.state.format, searchType: 'quick' } )}>
          <Text style={ Stylesheet.btnText }>{ item.ListName }</Text>
        </TouchableOpacity>
      </View>
      </View>
    );
