...
class sip2 {
  ...
  public $hostname     = SIP_HOSTNAME;
  public $port         = SIP_PORT; /* default sip2 port for Sirsi */
  public $library      = '';
...
# ****************************************************************************************************************************
  function parseLoginResponse($response) {
    $result['fixed'] = array('Ok'                => substr($response, 2, 1), );
    $result['variable'] = array();
    return $result;
  }

# ****************************************************************************************************************************
# * msgPatronInformation
# ****************************************************************************************************************************
  function msgPatronInformation($type, $start = '1', $end = '50') {

    /* According to the specification: Only one category of items should be  requested at a time, i.e. it would take 6 of these messages,
    ...
    //$this->_addVarOption('BQ', $end, true); /* old function version used padded 5 digits, this returns the upper bound of items checked out */
    
	return $this->_returnMessage();
  }

# ****************************************************************************************************************************
# * parsePatronInfoResponse
# ****************************************************************************************************************************
...
# ****************************************************************************************************************************
  function _parsevariabledata($response, $start) {
    ...
    foreach ($result['Raw'] as $item) {
      ...
      /* SD returns some odd values on ocassion, Unable to locate the purpose in spec, so I strip from the parsed array. Orig values will remain in ['raw'] element */
...
# ****************************************************************************************************************************
  function msgHold($mode, $expDate = '', $holdtype = '', $item = '', $title = '', $fee='N', $pkupLocation = '') {
    ...
    /* mode validity check: - remove hold, + place hold, * modify hold */
    if (strpos('-+*', $mode) === false) {
      $this->_debugmsg( "SIP2: Invalid hold mode: {$mode}");
      return false;
    }

    /* Valid hold types range from 1 - 9: 1=other; 2=any copy of title; 3=specific copy; 4=any copy at a single branch or location */
